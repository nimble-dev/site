...
The following code generates posterior samples from the random measure $G$. The `cMCMC` object includes the model and posterior samples from the parameters. The `getSamplesDPmeasure` function estimates a truncation level of $G$, namely `truncG`. The posterior samples are in a matrix with $(\text{truncG} \cdot (p+1))$ columns, where $p$ is the dimension of the vector of parameters with distribution $G$ (in this example $p=2$).
